...
risk — handled by the IR2104's internal dead-time and shoot-through interlock (the STM32
drives a single PWM input, not a complementary pair; see design_review_findings.md DR-1).
At 5 A, the low-side IRF540N (RDS(on) ~44 mΩ, ~0.22 V drop) saves ~0.9 W over a Schottky
(~0.4 V drop) — worth the extra FET above ~2 A, less compelling below. (An earlier draft
cited 8 mΩ, which is wrong for an IRF540N and overstated the saving.)
